...
public class SchrimpfV23 {
            ...
            CellStopPattern cellStopPattern)
    {
        ...
        result = new Result();
        result.setMethodUsed("Jsprit nearest to all V23");
        ...
        {
            MatrixElement to = NearestMap(wp,matrix,metro4Unused);
            WayPoint nearest = to.getWayPoint();
            if(matrix.containsKey(nearest)&&to.getDistance()<DistanceBetweenMap(nearest,wp,matrix))
            {
...
//WayPoint metro = metroOnly.get(0);
            ...
            for(WayPoint c_wp: toMetro.get(metro))
            {
                ...
                if(timeTo > maxHop) maxHop = timeTo;
            }

            maxHop *= 2.5; // <----------- SET K HERE
